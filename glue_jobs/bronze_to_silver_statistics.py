...
if initial_count == 0:
    ...
    if "snippet.title" in columns or "snippet__title" in columns:
        ...
        df = df.select(
            ...
            F.lit(datetime.utcnow().strftime(
                "%y.%d.%m")).alias("trending_date"),
    ...
    df = df.withColumn("engagement_rate",
                       F.when(
                           (F.col("views") > 0),
                           F.round((F.col("likes") + F.col("dislikes") +
                                    F.col("comment_count")) / F.col("views") * 100, 4)
                       ).otherwise(0.0)
                       )
    ...
    logger.info(
        f"After cleansing & dedup: {clean_count} records (removed {initial_count - clean_count})")
    ...
    for col_name in ["video_id", "title", "channel_title", "views"]:
        ...
        if null_count > 0:
            logger.warn(
                f"  DQ WARNING: {col_name} has {null_count} null values")

    negative_views = df.filter(F.col("views") < 0).count()
    if negative_views > 0:
        logger.warn(
            f"  DQ WARNING: {negative_views} records with negative views")
    ...
    sink.setCatalogInfo(catalogDatabase=SILVER_DB,
