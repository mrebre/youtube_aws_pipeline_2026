...
try:
    ...
    logger.warn(
        f"Could not load reference data: {e}. Proceeding without category names.")
...
sink1.setCatalogInfo(catalogDatabase=GOLD_DB,
...
channel = channel.withColumn(
    "rank_in_region", F.row_number().over(window_rank))
...
sink2.setCatalogInfo(catalogDatabase=GOLD_DB,
...
category = category.withColumn(
    "view_share_pct",
    F.round(F.col("total_views") /
            F.sum("total_views").over(window_total) * 100, 2)
)
...
sink3.setCatalogInfo(catalogDatabase=GOLD_DB,
